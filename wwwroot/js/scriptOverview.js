...
function initMap() {
    ...
    for (device in jsonDevicesEnvironment) {
        ...
        marker.bindPopup('<h5>device id:  ' + device + '</h5> <p> Last contact: ' + jsonDevicesEnvironment[device].lastContact + ' </p><p>Temp mean: ' + jsonDevicesEnvironment[device].tMean + ' °C</p><p>Total shock: ' + jsonDevicesEnvironment[device].totalShock + ' </p><p> Smax: ' + jsonDevicesEnvironment[device].sMax + ' </p><p> BatteryLvl: ' + jsonDevicesEnvironment[device].batteryLvl + '% </p>');
